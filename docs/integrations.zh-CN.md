...
| --- | --- | --- | --- | --- |
| Claude Code | `PreToolUse`，匹配 `Bash` | 返回 `permissionDecision: ask` | 交互模式下可由 Claude Code 显示确认 | 当前 cwd 可跨 action 延续；变量、alias 和函数从当前 action 分析 |
| Codex | `PreToolUse` 与 `PermissionRequest`，匹配 `Bash` | 默认在执行前阻断；`observe` 模式只记录并继续 | Caushell 默认直接阻断，Codex 原生权限流程可另行确认 | 每条 action 使用独立状态和本次请求提供的 cwd |
| DeepSeek Harness | 普通 Bash 的 `tools/pre-execute` | 映射为 `ask` | Web 与 ACP 可提供确认通道；Headless 没有通道时拒绝 | 普通 Bash 每次启动独立 shell；当前集成不支持 Persistent Bash |
...
交互模式可以把 `ask` 显示为用户确认。非交互环境能否继续执行，取决于 Claude Code 是否提供可用的审批通道和当前权限配置。
...
Web host 可以把 `ask` 发送到前端，ACP 可以转发给客户端。Headless profile 没有交互审批通道时，`ask` 最终会拒绝执行。DSH 的 `approval=never` 会把所有 `ask` 映射为拒绝。
...
三层按执行阶段依次工作：Caushell 完成语义决策，Harness 处理权限与确认，沙箱和内核约束进程的实际效果。工作区写入可以通过沙箱边界，同时因 Git 状态丢失等语义风险被 Caushell 要求确认。
