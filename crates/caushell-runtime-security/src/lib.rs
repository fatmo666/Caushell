...
fn harden_private_tree_inner(path: &Path) -> io::Result<()> {
    ...
    for entry in entries {
        ...
        if metadata.file_type().is_dir() {
            #[cfg(unix)]
            if let Err(error) = set_mode_if_needed(&entry_path, &metadata, PRIVATE_DIRECTORY_MODE) {
            ...
            if let Err(error) = validate_private_regular_metadata(&metadata, &entry_path) {
                if error.kind() == io::ErrorKind::NotFound {
                    continue;
                }
                return Err(error);
            ...
            #[cfg(unix)]
            if let Err(error) = set_mode_if_needed(&entry_path, &metadata, PRIVATE_FILE_MODE) {
                if error.kind() == io::ErrorKind::NotFound {
                    continue;
                }
                return Err(error);
            ...
            if metadata.file_type().is_socket() {
                if let Err(error) = harden_unix_socket_access(&entry_path, &metadata) {
                    if error.kind() == io::ErrorKind::NotFound {
                        continue;
                    }
                    return Err(error);
    ...
    #[cfg(unix)]
    {
        ...
        if metadata.nlink() == 0 {
            return Err(io::Error::new(
                ...
                format!(
                    "runtime file was replaced while being inspected: {}",
                    path.display()
                ),
            ));
        }
...
mod tests {
    ...
    fn private_tree_hardening_and_atomic_writes_tolerate_concurrent_replacement() {
        ...
        let record_path = nested.join("active-session.json");
        write_private_file_atomic(&record_path, b"{}\n").unwrap();
            ...
            writers.push(thread::spawn(move || {
                ...
                for iteration in 0..24 {
                    write_private_file_atomic(
                        &record_path,
                        format!("{{\"writer\":{writer_id},\"iteration\":{iteration}}}\n"),
                    )
                    .unwrap();
                }
            }));
        ...
        let record = String::from_utf8(read_private_file(&record_path).unwrap()).unwrap();
        assert!(record.starts_with("{\"writer\":"));
