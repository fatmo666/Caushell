...
fn posttooluse_drains_large_payload_before_exiting() {
    let root = unique_temp_dir("caushell-codex-posttooluse");
    ...
    let session_id = "codex-posttooluse-regression";
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_caushell-codex-hook"))
        .arg("PostToolUse")
        .env("HOME", &home)
        .env("XDG_STATE_HOME", &state)
        .env("XDG_RUNTIME_DIR", &runtime)
        .env("CAUSHELL_CONFIG_PATH", root.join("config.yaml"))
        .env("CODEX_PLUGIN_OPTION_RUNTIME_PATH", &runtime_binary)
        .env("CODEX_PLUGIN_OPTION_SOCKET_ROOT", &socket_root)
        .env("CODEX_PLUGIN_OPTION_STORE_ROOT", &store)
