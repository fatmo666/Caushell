...
fn run_direct_hook_smoke(
    ...
    let post_payload = codex_like_post_hook_payload(&session_id, &cwd);
    let post_output = match run_hook_event(&hook_path, "PostToolUse", &post_payload) {
        Ok(output) => output,
        Err(error) => {
            report.fail(format!(
    ...
    check_smoke_log_session_id(&new_log, "PostToolUse", &session_id, report);
}
...
fn codex_like_post_hook_payload(session_id: &str, cwd: &str) -> String {
    serde_json::json!({
        "session_id": session_id,
        "cwd": cwd,
...
fn run_hook_event(hook_path: &Path, event_name: &str, stdin_text: &str) -> io::Result<Output> {
    ...
    let write_result = stdin.write_all(stdin_text.as_bytes());
    drop(stdin);
...
fn check_smoke_log_session_id(
    new_log: &str,
    event_name: &str,
    session_id: &str,
    report: &mut DoctorReport,
) {
    let event_field = format!("event={event_name}");
    let session_field = format!("session_id={session_id}");
    let found = new_log.lines().any(|line| {
        line.split_whitespace().any(|field| field == event_field)
            && line.split_whitespace().any(|field| field == session_field)
    });

    if found {
        report.ok(format!(
            "hook log contains event={event_name} with smoke session_id"
        ));
    } else {
        report.fail(format!(
            "hook log does not contain event={event_name} with session_id={session_id}; PostToolUse payload was not consumed"
        ));
    }
}
