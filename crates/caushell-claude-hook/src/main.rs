...
struct RuntimePaths {
    ...
    active_sessions_lock_path: PathBuf,
}
...
impl HookContext {
    fn new(event_name: String, session_id: String) -> Result<Self, HookError> {
        ...
        ensure_private_directory(&config.runtime_security_root).map_err(|error| {
            hook_path_io_error(
                "ensure runtime security root",
                &config.runtime_security_root,
                error,
            )
        })?;
        ensure_private_directory_tree(&config.runtime_security_root, &paths.workspace_runtime_dir)
            .map_err(|error| {
                hook_path_io_error(
                    "ensure workspace runtime directory",
                    &paths.workspace_runtime_dir,
                    error,
                )
            })?;
        harden_private_tree(&paths.workspace_runtime_dir).map_err(|error| {
            hook_path_io_error(
                "harden workspace runtime tree",
                &paths.workspace_runtime_dir,
                error,
            )
        })?;
        ensure_private_directory(&config.store_root).map_err(|error| {
...
impl RuntimePaths {
    fn new(config: &HookConfig) -> Self {
        ...
        Self {
            ...
            active_sessions_lock_path: workspace_runtime_dir.join("active-sessions.lock"),
...
fn run_pretooluse<W: Write>(
    ...
    if let Err(error) = write_active_session_record(&context) {
        let _ = context.log(
            "warn",
            "failed to update active session record",
            &[
                ("error_code", "active_session_update_failed".to_string()),
...
fn run_status() -> Result<(), HookError> {
    ...
    println!(
        "active_sessions_lock_path={}",
        path_string(&context.paths.active_sessions_lock_path)
    );
    println!(
        "plugin_log_path={}",
...
fn write_active_session_record(context: &HookContext) -> Result<(), HookError> {
    ...
    let _lock = acquire_exclusive_lock(&context.paths.active_sessions_lock_path)?;
    remove_legacy_session_start_record(context)?;
    prune_stale_active_sessions_locked(context)?;
    ...
    write_private_file_atomic(&path, [payload, b"\n".to_vec()].concat())?;
    Ok(())
}
...
fn prune_stale_active_sessions_locked(context: &HookContext) -> Result<(), HookError> {
    let now_ms = current_time_ms()?;
    ...
    for entry in entries.flatten() {
        ...
        let Some(contents) = read_private_file_optional(&record_path)? else {
            remove_dir_if_empty(&dir)?;
            continue;
        };
        let record: ActiveSessionRecord = match serde_json::from_slice(&contents) {
            Ok(record) => record,
            // Preserve malformed records. A prior hook may have been interrupted while
            // writing, and removing it would race a still-running older hook.
            Err(_) => continue,
        };
        let stale = now_ms.saturating_sub(record.heartbeat_at_ms) > stale_after_ms;
        if stale {
...
fn hook_path_io_error(operation: &str, path: &Path, error: io::Error) -> HookError {
    HookError::Io(io::Error::new(
        error.kind(),
...
mod tests {
    use super::{
        ClaudeHookRequest, ClaudeToolInput, DaemonMetadata, DaemonStatus, HookConfig, HookContext,
        RuntimePaths, active_session_record_path, build_runtime_request, civil_from_days,
        daemon_process_matches, emit_pre_context_fallback_response,
        emit_pretooluse_protocol_error_response, major_minor, prune_stale_active_sessions_locked,
        read_active_session_record, runtime_root_for, session_id_from_payload, user_visible_reason,
        version_compatibility_status, workspace_hash, write_active_session_record,
    };
    ...
    fn active_session_test_context(root: PathBuf, session_id: &str) -> HookContext {
        ensure_private_directory(&root).expect("test runtime root should be private");
        let config = HookConfig {
            runtime_path: root.join("caushell"),
            runtime_fingerprint: "test-runtime".to_string(),
        ...
        let paths = RuntimePaths::new(&config);
        ensure_private_directory_tree(&root, &paths.workspace_runtime_dir)
            .expect("test workspace runtime directory should be private");
        HookContext {
    ...
    fn unique_active_session_test_root(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("clock should be after Unix epoch")
            .as_nanos();
        std::env::temp_dir().join(format!(
            "caushell-claude-{label}-{}-{nanos}",
            std::process::id()
        ))
    }

    #[test]
    fn active_session_updates_are_serialized_and_atomic() {
        let root = unique_active_session_test_root("active-session-concurrency");
        let context = active_session_test_context(root.clone(), "concurrent-session");
        ...
        for _ in 0..16 {
            ...
            workers.push(thread::spawn(move || {
                ...
                for _ in 0..4 {
                    write_active_session_record(&context)
                        .expect("concurrent active session update should succeed");
                }
            }));
        }
        ...
        for worker in workers {
            worker
                .join()
                .expect("active session worker should not panic");
        }

        let record = read_active_session_record(&active_session_record_path(&context))
            .expect("active session record should be readable")
            .expect("active session record should exist");
        assert_eq!(record.session_id, "concurrent-session");
        assert_eq!(record.last_event_name, "PreToolUse");

        std::fs::remove_dir_all(root).expect("test runtime root should be removable");
    }

    #[test]
    fn active_session_pruning_preserves_malformed_records() {
        let root = unique_active_session_test_root("active-session-malformed");
        let context = active_session_test_context(root.clone(), "current-session");
        let malformed_dir = context.paths.active_sessions_dir.join("malformed");
        ensure_private_directory_tree(&context.config.runtime_security_root, &malformed_dir)
            .expect("malformed record directory should be private");
        let malformed_path = malformed_dir.join("active-session.json");
        write_private_file(&malformed_path, b"{").expect("malformed test record should be written");

        let lock = open_private_read_write(&context.paths.active_sessions_lock_path)
            .expect("active session lock should open");
        super::flock_exclusive(&lock, false).expect("active session lock should be held");
        prune_stale_active_sessions_locked(&context)
            .expect("pruning malformed active session records should succeed");
        drop(lock);
